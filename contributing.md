...
* Avoid adding library dependencies where possible. Querqy Solr is plugged into Solr. The fewer dependencies we add, the farther away
  we stay from jar hell.
* Use 4 spaces for indentation. Opening `{` should be put on the same line the like statement that this code block belongs to:
```
void myMethod() {
    if (isSunday()) {
